...
namespace Lykke.Service.ConfirmationCodes.Core.Services
{
    ...
    {
        Task<string> SendConfirmEmail(string email, string partnerId, bool isPriority, int codeLength);
        //todo: consider methods combining for Cyp and Non-cyp
        Task<string> SendConfirmCypEmail(string email, string partnerId, bool isPriority, int codeLength);
